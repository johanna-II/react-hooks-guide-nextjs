...
export function useOptimizedTranslations(): TranslateFunction {
  ...
  const t = useCallback(
    (key: string, defaultText?: string): string => {
      ...
      const globalCached = globalTranslationCache.get(cacheKey);
      ...
      if (!batchQueueRef.current.has(key)) {
        ...
        batchTimerRef.current = setTimeout(processBatch, 10); // 더 빠른 응답을 위해 10ms로 단축
      }
  ...
  // Rich text 지원
  (t as TranslateFunction).rich = useCallback(
    (key: string, values: Record<string, (chunks: React.ReactNode) => React.ReactNode>) => {
      const text = t(key);
  ...
  // 초기 로딩 시 모든 번역 미리 요청
  useEffect(() => {
    if (locale === 'ko' || hasInitializedRef.current) return;

    hasInitializedRef.current = true;

    // 서버에서 이미 번역된 키들을 제외하고 나머지 모두 요청
    const keysToTranslate = Object.keys(koreanTexts).filter((key) => {
      // 서버 번역이 있거나 이미 클라이언트에서 번역된 경우 제외
      return (
        !serverTranslations?.translations?.[key] &&
        !clientTranslations[key] &&
        !globalTranslationCache.has(`${locale}:${key}`)
      );
    });

    if (keysToTranslate.length > 0) {
      // 배치로 모든 키 추가
      keysToTranslate.forEach((key) => batchQueueRef.current.add(key));
      // 즉시 처리
      processBatch();
    }
  }, [locale, serverTranslations, clientTranslations, processBatch]);

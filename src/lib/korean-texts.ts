export const koreanTexts: Record<string, string> = {
  ...
  'hero.description':
    'React Hooks의 모든 것을 배우고, React 19의 혁신적인 기능들을 체험해보세요. 실시간 데모와 함께하는 인터랙티브 학습 가이드입니다.',
  ...
  'guide.whatIsHookDesc':
    'Hook은 함수형 컴포넌트에서 React의 상태와 생명주기 기능을 사용할 수 있게 해주는 함수입니다. 클래스 컴포넌트 없이도 React의 모든 기능을 활용할 수 있게 되었습니다.',
  'guide.whyUseHooks': '왜 Hook을 사용할까?',
  'guide.whyUseHooksDesc':
    'Hook을 사용하면 컴포넌트 간에 상태 로직을 재사용하기 쉽고, 복잡한 컴포넌트를 더 쉽게 이해할 수 있습니다.',
  'guide.hookAdvantages': 'Hook의 장점',
  'guide.hookAdvantagesDesc':
    '클래스 컴포넌트의 this 바인딩 문제가 없고, 컴포넌트를 더 작은 함수로 나누어 테스트하기 쉽습니다.',
  ...
  'mobile.learnEverything':
    'React Hooks의 모든 것을 배우고, React 19의 혁신적인 기능들을 체험해보세요.',

  // Form
  'form.title': 'Form Actions 데모',
  'form.description':
    '이 데모는 React 19의 새로운 Form Actions 기능을 시뮬레이션합니다. 실제 Form Actions에서는 useActionState와 useFormStatus를 사용합니다.',
  ...
  'whyHooks.wrapperHell.detail':
    '마치 러시아 인형처럼 겹겹이 싸인 코드를 풀어서 깔끕하게 정리해줍니다.',
  'whyHooks.lifecycleSimplification.title': 'Lifecycle 단순화',
  'whyHooks.lifecycleSimplification.desc':
    '여러 lifecycle method에 흩어진 logic을 useEffect 하나로 통합합니다.',
  'whyHooks.lifecycleSimplification.detail':
    '출생, 성장, 소멸 같은 복잡한 생명주기를 하나의 함수로 간단하게 관리합니다.',
  'whyHooks.thisRemoval.title': 'this 제거',
  'whyHooks.thisRemoval.desc':
    'JavaScript의 혼란스러운 this binding 없이 깔끕한 functional component를 작성합니다.',
  'whyHooks.thisRemoval.detail': '누가 "나"인지 헷갈리지 않고 명확하게 코드를 작성할 수 있습니다.',
  'whyHooks.logicReuse.title': 'Logic 재사용',
  'whyHooks.logicReuse.desc':
    'Custom Hook으로 상태 관련 로직을 쉽게 재사용하고 공유할 수 있습니다.',
  'whyHooks.logicReuse.detail':
    '레고 블록처럼 필요한 기능을 조립하고, 다른 프로젝트에서도 똑같이 사용할 수 있습니다.',
  'whyHooks.typescriptFriendly.title': 'TypeScript 친화적',
  'whyHooks.typescriptFriendly.desc':
    '타입 추론과 제네릭을 활용하여 타입 안전한 코드를 작성할 수 있습니다.',
  'whyHooks.typescriptFriendly.detail':
    '런타임 에러를 컴파일 타임에 미리 잡아내어 더욱 안정적인 애플리케이션을 구축할 수 있습니다.',
  'whyHooks.performanceOptimization.title': '성능 최적화',
  'whyHooks.performanceOptimization.desc':
    'React.memo, useMemo, useCallback으로 불필요한 리렌더링을 방지합니다.',
  'whyHooks.performanceOptimization.detail':
    '메모이제이션을 통해 계산 결과를 재사용하여 사용자 경험을 크게 향상시킵니다.',
  ...
  'whyHooks.items.simpleComponents.desc':
    '함수형 컴포넌트는 클래스 컴포넌트보다 이해와 테스트가 쉽다',
  ...
  'optimization.metrics.description':
  ...
  'optimization.renderCount.memo.desc':
  ...
  'optimization.renderCount.propsReuseEfficient':
  ...
  'optimization.memoryManagement.useMemoDesc':
    'useEffect cleanup으로 memory leak을 방지합니다. 사용하지 않는 것을 계속 메모리에 두면 앱이 느려집니다.',
  ...
  'optimization.reactMemo.countChangedExpensiveValueSame':
    'Count가 바뀌어도 expensiveValue는 그대로',
  'optimization.reactMemo.parentChildBothRerendered': '부모+자식 모두 리렌더링',
  'optimization.reactMemo.expensiveValueChangedUseMemoRerun':
  ...
  'optimization.reactMemo.coreExplanation':
    '부모 컴포넌트가 리렌더링되어도 자식은 props가 변경되지 않으면 리렌더링되지 않습니다.',
  'optimization.reactMemo.tryItBelow': '아래에서 직접 체험해보세요!',
  'optimization.reactMemo.detailedExplanation':
    '<span>Count 버튼</span>을 클릭하면 부모 컴포넌트만 리렌더링되고, <span>Expensive 버튼</span>을 클릭하면 부모와 자식 컴포넌트 모두 리렌더링됩니다. React.memo는 props가 변경되지 않으면 이전 렌더링 결과를 재사용하여 불필요한 계산을 방지합니다. 마치 캐시처럼 작동하여 성능을 크게 향상시킵니다.',
  ...
  'optimization.uiResponsiveness.memo.desc':
    '무거운 연산과 UI 업데이트를 분리하여 반응성을 유지합니다.',
  ...
  'optimization.memoryManagement.memo.desc':
    '이벤트 리스너와 타이머를 적절히 정리하여 메모리 누수를 방지합니다.',
  ...
  'form.actionDescription':
    '<strong>useActionState</strong>와 <strong>Form Actions</strong>로 폼 처리가 훨씬 간단해졌습니다. 자동 폼 리셋, 에러 처리, pending 상태를 자동으로 관리합니다.',
  ...
  'mobile.whatIsHook.desc':
    '함수형 컴포넌트에서 React의 상태와 생명주기 기능을 사용할 수 있게 해주는 함수입니다.',
  'mobile.whyUseHooks': '왜 Hook을 사용할까?',
  'mobile.whyUseHooks.desc':
    '컴포넌트 간에 상태 로직을 재사용하기 쉽고, 복잡한 컴포넌트를 더 쉽게 이해할 수 있습니다.',
  'mobile.hookAdvantages': 'Hook의 장점',
  'mobile.hookAdvantages.desc':
    '클래스 컴포넌트의 this 바인딩 문제가 없고, 컴포넌트를 더 작은 함수로 나누어 테스트하기 쉽습니다.',

  // React Hooks Guide Additional
  'guide.whenToUse': '언제 사용할까?',
  'guide.whenToUse.desc':
    '함수형 컴포넌트에서 상태 관리, 사이드 이펙트 처리, 또는 다른 React 기능을 사용할 때 필요합니다.',
  'guide.whenToUse.desc2':
    '함수형 컴포넌트에서 상태 관리, 사이드 이펙트 처리, 컴포넌트 간 로직 공유가 필요할 때 사용합니다.',
  'guide.coreHooks.title': '핵심 Hooks 완벽 가이드',
  'guide.coreHooks.description':
    'React의 기본 Hooks들을 실시간 데모와 함께 학습하세요. 각 Hook의 동작 원리와 실제 사용법을 체험할 수 있습니다.',
  'guide.rules.title': 'Hooks 사용 규칙',
  'guide.optimization.title': '성능 최적화 마스터하기',
  'guide.optimization.description':
    'React.memo, useMemo, useCallback 등을 활용한 성능 최적화 기법을 실시간 데모와 함께 학습하세요.',
  'guide.advanced.title': '고급 패턴 실험실',
  'guide.advanced.description':
    'useCallback, useRef, useEffect 등 고급 Hook 패턴들을 실시간으로 실험해보세요. 각 패턴의 동작 원리와 최적화 효과를 직접 확인할 수 있습니다.',
  ...
  'guide.rules.whyDesc':
    'React는 Hook의 호출 순서에 의존하여 상태를 올바르게 연결합니다. 조건부 Hook 호출은 이 순서를 깨뜨리고 버그를 유발합니다.',
  ...
  'react19.newFeatures.description':
    'React 19에서는 개발자 경험을 크게 향상시키는 새로운 Hook들과 기능들이 추가되었습니다. 특히 Actions, Form Actions, use() Hook 등이 핵심입니다.',
  ...
  'react19.actions.description':
    'React 19의 <strong>Actions</strong>는 async 함수를 transitions에서 사용할 수 있게 해줍니다.',
  'react19.actions.desc':
    '사용자 경험을 해치지 않고 비동기 상태 업데이트를 처리합니다. 폼 제출, 데이터 저장, 페이지 전환 등의 비동기 작업을 우아하게 처리할 수 있습니다.',
  'react19.actions.autoManagement':
    'pending 상태, 에러 처리, 낙관적 업데이트를 자동으로 관리합니다.',
  'react19.formActions.simplifiedDesc':
    '폼 처리가 훨씬 간단해졌습니다. 자동 폼 리셋, 에러 처리, pending 상태를 자동으로 관리합니다.',
  'react19.formActions.withUseActionState':
    '<strong>useActionState</strong>와 <strong>Form Actions</strong>로',
  ...
  'react19.use.description':
    '새로운 <strong>use() Hook</strong>으로 Promise를 직접 읽을 수 있습니다. 조건문 안에서도 사용 가능한 유일한 Hook입니다.',
  'react19.use.desc':
    '조건부로 Promise를 읽고, Context를 조건부로 사용할 수 있습니다. Suspense와 함께 사용하여 비동기 데이터를 선언적으로 사용할 수 있습니다.',
  ...
  'react19.formHooks.description':
    '<strong>useFormStatus</strong>로 폼의 pending 상태를 쉽게 관리하고, <strong>useOptimistic</strong>으로 즉시 UI 업데이트를 구현할 수 있습니다.',
  'react19.formHooks.desc':
    '폼의 pending 상태를 추적하고 낙관적 업데이트를 수행합니다. 서버 응답을 기다리는 동안 UI를 즉시 업데이트할 수 있습니다.',
  ...
  'react19.compiler.description':
    'React 19의 새로운 컴파일러는 <strong>useMemo</strong>, <strong>useCallback</strong>, <strong>React.memo</strong>를 자동으로 적용합니다. 개발자가 수동으로 최적화하지 않아도 React가 알아서 처리합니다.',
  'react19.compiler.desc':
    'React Compiler가 코드를 분석하고 자동으로 최적화합니다. useMemo, useCallback, React.memo를 수동으로 추가할 필요가 없어집니다.',
  ...
  'react19.resources.description':
    '<strong>prefetchDNS</strong>, <strong>preconnect</strong>, <strong>preload</strong>, <strong>preinit</strong> 등 새로운 리소스 프리로딩 API들로 페이지 성능을 크게 향상시킬 수 있습니다.',
  'react19.resources.desc':
    '리소스를 사전에 로드하여 성능을 향상시킵니다. 다양한 레벨의 프리로딩을 통해 사용자 경험을 개선할 수 있습니다.',
  ...
  'optimization.metrics.renderCount.desc':
    '적을수록 좋습니다. 마치 그림을 그릴 때 전체를 지우고 다시 그리는 것보다 필요한 부분만 수정하는 것이 효율적입니다.',
  'optimization.metrics.executionTime': '계산에 걸리는 시간',
  'optimization.metrics.executionTime.desc':
    '복잡한 계산 결과를 저장해두면 매번 다시 계산하지 않아도 됩니다. 계산기 결과를 메모해두는 것과 같습니다.',
  'optimization.metrics.uiResponsiveness': '사용자 입력 반응성',
  'optimization.metrics.uiResponsiveness.desc':
    '타이핑할 때 글자가 바로 나타나야 사용하기 편합니다. 스마트폰이 터치에 바로 반응하는 것처럼 빨라야 합니다.',
  'optimization.metrics.memoryManagement': '메모리 사용량',
  'optimization.metrics.memoryManagement.desc':
    '사용하지 않는 것들을 제때 정리해야 합니다. 책상 정리를 하지 않으면 공간이 부족해지는 것과 같습니다.',
  'optimization.memoryManagement.explanation':
    '사용하지 않는 것은 정리해야 메모리가 부족하지 않습니다. 마치 집에 쓰레기를 계속 쌓아두면 공간이 부족해지는 것과 같습니다.',
  'optimization.uiResponsiveness.redrawEveryTyping':
  ...
  'optimization.useMemo.guideDescription':
    '의존성이 변경되지 않으면 계산을 다시 하지 않습니다. Expensive Value를 변경해보면 계산이 실행되는 것을 Console에서 확인할 수 있습니다.',
  'optimization.useMemo.consoleMessage': 'Console 메시지:',
  'optimization.useMemo.performanceTip': '💡 성능 팁:',
  'optimization.useMemo.performanceTipDesc':
    'useMemo는 계산 결과를 "기억"합니다. expensiveValue가 변경되지 않으면 이전 계산 결과를 재사용하여 불필요한 연산을 방지합니다.',
  ...
  'optimization.guide.keyPointDesc':
    'React에서 key는 컴포넌트의 "신분증" 역할을 합니다. 고유한 key를 사용하면 React가 어떤 항목이 변경되었는지 정확히 파악하여 불필요한 리렌더링을 방지할 수 있습니다.',
